...
#set directory
setwd("~/Dropbox/Research/Projects/Methods-Sampling/Pilot4-Brightness")

files = list.files(path = ".", pattern = '.csv')

data1 = read.table(file=files[1],header=T,sep=",") %>% mutate(emphasis="accuracy")

#The first 33 subjects are either Gina's tests or participants from Pilot 2.
#Participants in pilot 4 are 1284 and above.

data = data1 %>% 
  filter(resultid>1284) %>% 
