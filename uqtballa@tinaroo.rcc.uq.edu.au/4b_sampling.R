...
load(paste0("starting_values_exp2_",source_,"_",wave_,".RData"))

#Gets rid of bad chains
system.time({unstuck_samples  <- h.run.unstuck.dmc(starting_samples, p.migrate = .05,h.p.migrate=.05, cores = 8)})
...
system.time({converged_samples <- h.run.converge.dmc(h.samples.dmc(nmc=100, samples=unstuck_samples), 
                                                 nmc=100,cores=8,finalrun=TRUE,finalI=500)})

save(converged_samples,file=paste0("final_samples_exp2_",source_,"_",wave_,".RData"))
